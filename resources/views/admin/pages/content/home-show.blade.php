            ...
            <div class="page-header">
                <div class="page-header-left d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Detail Konten Home</h5>
                    </div>
                    <ul class="breadcrumb">
                ...
                <div class="page-header-right ms-auto">
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalHeroBanner">
                    ...
                    <div class="col-12">
                        <div class="card stretch stretch-full">
                            <div class="card-header">
                                <h5 class="card-title">Konten Home</h5>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped align-middle mb-0">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>1</td>
                                                <td class="fw-semibold">Hero Banner</td>
                                                <td class="text-muted small">Gambar banner utama pada halaman Home.</td>
                                                <td>
                                                    @if($settings->hero_image_path)
                                                        <img src="{{ asset('storage/' . $settings->hero_image_path) }}" style="height: 60px; width: 120px; object-fit: cover;" alt="Hero Banner">
                                                    @else
                                            ...
                                            <tr>
                                                <td>2</td>
                                                <td class="fw-semibold">Banner Tentang Kami</td>
                                                <td class="text-muted small">Gambar section "Gerakan Anak Muda yang Berdaya".</td>
                                                <td>
                                                    @if($settings->about_image_path)
                                                        <img src="{{ asset('storage/' . $settings->about_image_path) }}" style="height: 60px; width: 120px; object-fit: cover;" alt="Tentang Kami">
                                                    @else
                                                        <span class="text-muted small">Tidak ada gambar</span>
                                                    @endif
                                                </td>
                                            ...
                                            <tr>
                                                <td>5</td>
                                                <td class="fw-semibold">{{ $settings->impact_1_title ?? 'Penerima Manfaat' }}</td>
                                                <td class="text-muted small">{{ $settings->impact_1_text ?? 'Artikel eksternal untuk Penerima Manfaat.' }}</td>
                                                <td>
                                                    <div class="small fw-semibold">{{ $settings->impact_1_number ?? '-' }}</div>
                                                    <div class="text-muted small">{{ $settings->impact_1_date ?? '-' }}</div>
                                                    @if(!empty($settings->impact_1_url))
                                                        <div class="mt-1">
                                                            <a href="{{ $settings->impact_1_url }}" target="_blank" class="small">Link Artikel</a>
                                                        </div>
                                                    @endif
                                                </td>
                                                <td>
                                            ...
                                            <tr>
                                                <td>6</td>
                                                <td class="fw-semibold">{{ $settings->impact_2_title ?? 'Volunteer' }}</td>
                                                <td class="text-muted small">{{ $settings->impact_2_text ?? 'Artikel eksternal untuk Volunteer.' }}</td>
                                                <td>
                                                    <div class="small fw-semibold">{{ $settings->impact_2_number ?? '-' }}</div>
                                                    <div class="text-muted small">{{ $settings->impact_2_date ?? '-' }}</div>
                                                    @if(!empty($settings->impact_2_url))
                                                        <div class="mt-1">
                                                            <a href="{{ $settings->impact_2_url }}" target="_blank" class="small">Link Artikel</a>
                                                        </div>
                                                    @endif
                                                </td>
                                                <td>
                                            ...
                                            <tr>
                                                <td>7</td>
                                                <td class="fw-semibold">{{ $settings->impact_3_title ?? 'Bantuan Tersalurkan' }}</td>
                                                <td class="text-muted small">{{ $settings->impact_3_text ?? 'Artikel eksternal untuk Bantuan Tersalurkan.' }}</td>
                                                <td>
                                                    <div class="small fw-semibold">{{ $settings->impact_3_number ?? '-' }}</div>
                                                    <div class="text-muted small">{{ $settings->impact_3_date ?? '-' }}</div>
                                                    @if(!empty($settings->impact_3_url))
                                                        <div class="mt-1">
                                                            <a href="{{ $settings->impact_3_url }}" target="_blank" class="small">Link Artikel</a>
                                                        </div>
                                                    @endif
                                                </td>
                                                <td>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ['modalHeroBanner', 'modalAboutBanner', 'modalAuthor', 'modalImpact'].forEach(function (id) {
